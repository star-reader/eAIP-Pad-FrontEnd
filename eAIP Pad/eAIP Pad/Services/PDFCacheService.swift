...
class PDFCacheService {
    ...
    func saveToCache(pdfData: Data, airacVersion: String, documentType: String, id: String) throws {
        ...
        LoggerService.shared.info(
            module: "PDFCacheService", message: "保存 PDF 到缓存: \(documentType)_\(id)")
    }
    ...
    func getCurrentAIRACVersion(modelContext: ModelContext) -> String? {
        do {
            ...
            LoggerService.shared.error(
                module: "PDFCacheService", message: "获取当前 AIRAC 版本失败: \(error.localizedDescription)"
            )
            return nil
        }
    }
    ...
    func clearCacheForVersion(_ version: String) {
        ...
        do {
            ...
            LoggerService.shared.error(
                module: "PDFCacheService", message: "清理缓存失败: \(error.localizedDescription)")
        }
    }
        ...
        do {
            ...
            for versionDir in cacheContents where versionDir.hasDirectoryPath {
                ...
                if versionName != currentVersion {
                    ...
                    LoggerService.shared.info(
                        module: "PDFCacheService", message: "已清理旧版本缓存: \(versionName)")
                }
            }
            LoggerService.shared.info(module: "PDFCacheService", message: "旧版本缓存清理完成")
        } catch {
            LoggerService.shared.error(
                module: "PDFCacheService", message: "清理旧版本缓存失败: \(error.localizedDescription)")
        }
    }
    ...
    func clearAllCache() {
        ...
        do {
            ...
            LoggerService.shared.error(
                module: "PDFCacheService", message: "清理所有缓存失败: \(error.localizedDescription)")
        }
    }
    ...
    private func calculateDirectorySize(url: URL) -> Int64 {
        ...
        do {
            ...
            LoggerService.shared.error(
                module: "PDFCacheService", message: "计算文件大小失败: \(error.localizedDescription)")
        }
    ...
    func getCacheStatistics() -> [String: Int] {
        ...
        do {
            ...
            LoggerService.shared.error(
                module: "PDFCacheService", message: "获取缓存统计失败: \(error.localizedDescription)")
        }
    ...
    func clearDataCache() {
        ...
        do {
            ...
            LoggerService.shared.error(
                module: "PDFCacheService", message: "清理数据缓存失败: \(error.localizedDescription)")
        }
    }
    ...
    func clearDataCacheForVersion(_ version: String) {
        ...
        do {
            try fileManager.removeItem(at: versionDirectory)
            LoggerService.shared.info(
                module: "PDFCacheService", message: "已清理 AIRAC \(version) 的数据缓存")
        } catch {
            LoggerService.shared.error(
                module: "PDFCacheService", message: "清理数据缓存失败: \(error.localizedDescription)")
        }
        ...
        do {
            ...
            for versionDir in cacheContents where versionDir.hasDirectoryPath {
                ...
                if versionName != currentVersion {
                    try fileManager.removeItem(at: versionDir)
                    LoggerService.shared.info(
                        module: "PDFCacheService", message: "已清理旧版本数据缓存: \(versionName)")
                }
            }
            LoggerService.shared.info(module: "PDFCacheService", message: "旧版本数据缓存清理完成")
        } catch {
            LoggerService.shared.error(
                module: "PDFCacheService", message: "清理旧版本数据缓存失败: \(error.localizedDescription)")
        }
    }
}
...
extension PDFCacheService {
    enum DataType {
        ...
        static let enrouteCharts = "enroute"
        static let aipDocuments = "aip"
        static let supDocuments = "sup"
        static let amdtDocuments = "amdt"
        static let notamDocuments = "notam"
    }
}
