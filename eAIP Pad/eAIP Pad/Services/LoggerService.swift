...
class LoggerService {
    ...
    private let osLog = Logger(
        subsystem: Bundle.main.bundleIdentifier ?? "eAIP-Pad", category: "LoggerService")
        ...
        return try await withCheckedThrowingContinuation { continuation in
            queue.async { [weak self] in
                guard let self = self else {
                    continuation.resume(
                        throwing: NSError(
                            domain: "LoggerService", code: -1,
                            userInfo: [NSLocalizedDescriptionKey: "LoggerService 实例不存在"]))
                    return
                }
