...
class AIRACService: ObservableObject {
    ...
    func checkAndUpdateAIRAC(modelContext: ModelContext) async {
        LoggerService.shared.log(
            type: .info, module: "AIRACService", message: "checkAndUpdateAIRAC started")
        isUpdating = true
        updateProgress = 0.0
        ...
        do {
            ...
            LoggerService.shared.log(
                type: .info, module: "AIRACService",
                message: "successfully got airac response: \(airacResponse)")
            updateProgress = 0.2
            ...
            LoggerService.shared.log(
                type: .info, module: "AIRACService",
                message: "existingVersions: \(existingVersions)")
            if existingVersions.isEmpty {
                // 创建新版本记录
                let newVersion = AIRACVersion(
                    version: airacResponse.version,
                    effectiveDate: ISO8601DateFormatter().date(from: airacResponse.effectiveDate)
                        ?? Date(),
                    isCurrent: airacResponse.isCurrent
                )
                modelContext.insert(newVersion)
                LoggerService.shared.log(
                    type: .info, module: "AIRACService", message: "newVersion: \(newVersion)")
                // 将其他版本标记为非当前版本
                ...
                LoggerService.shared.log(
                    type: .info, module: "AIRACService", message: "allVersions: \(allVersions)")
                updateProgress = 0.4
                ...
                LoggerService.shared.log(
                    type: .info, module: "AIRACService",
                    message: "start to download charts for version: \(airacResponse.version)")
                await downloadChartsForVersion(airacResponse.version, modelContext: modelContext)
                LoggerService.shared.log(
                    type: .info, module: "AIRACService", message: "charts downloaded successfully")
                try modelContext.save()
                LoggerService.shared.log(
                    type: .info, module: "AIRACService", message: "saved successfully")
                updateProgress = 1.0
                updateMessage = "更新完成"
            ...
            LoggerService.shared.log(
                type: .info, module: "AIRACService", message: "checkAndUpdateAIRAC completed")
        } catch {
            errorMessage = "更新失败: \(error.localizedDescription)"
            LoggerService.shared.log(
                type: .error, module: "AIRACService",
        ...
        isUpdating = false
        LoggerService.shared.log(
            type: .info, module: "AIRACService", message: "isUpdating set to false")
    }
    ...
    private func downloadChartsForVersion(_ version: String, modelContext: ModelContext) async {
        do {
            LoggerService.shared.log(
                type: .info, module: "AIRACService",
                message: "downloadChartsForVersion started: \(version)")
            ...
            LoggerService.shared.log(
                type: .info, module: "AIRACService", message: "airports: \(airports)")
            var totalCharts = 0
            ...
            for airport in airports {
                ...
                LoggerService.shared.log(
                    type: .info, module: "AIRACService", message: "charts: \(charts)")
            }

            updateMessage = "下载航图数据..."
            LoggerService.shared.log(
                type: .info, module: "AIRACService",
                message: "start to download charts for airports")
            // 下载并保存航图数据
            for (airportIndex, airport) in airports.enumerated() {
                ...
                for chart in charts {
                    ...
                    LoggerService.shared.log(
                        type: .info, module: "AIRACService",
                        message: "existingCharts: \(String(describing: existingCharts))")
                    ...
                    LoggerService.shared.log(
                        type: .info, module: "AIRACService",
                        message: "updateProgress: \(updateProgress)")
                    LoggerService.shared.log(
                        type: .info, module: "AIRACService",
            ...
            LoggerService.shared.log(
                type: .error, module: "AIRACService",
                message: "downloadChartsForVersion failed: \(error.localizedDescription)")
        }
    }
    ...
    func cleanupOldVersions(modelContext: ModelContext) async {
        do {
            ...
            if allVersions.count > 3 {
                ...
                LoggerService.shared.log(
                    type: .info, module: "AIRACService",
                    message: "versionsToDelete: \(versionsToDelete)")
                for version in versionsToDelete {
                    ...
                    LoggerService.shared.log(
                        type: .info, module: "AIRACService",
                        message: "chartsToDelete: \(chartsToDelete)")
                    for chart in chartsToDelete {
                        modelContext.delete(chart)
                    }
                    LoggerService.shared.log(
                        type: .info, module: "AIRACService", message: "version deleted: \(version)")
                    // 删除版本记录
    ...
    private func cleanupFileCache() async {
        LoggerService.shared.log(
            type: .info, module: "AIRACService", message: "cleanupFileCache started")
        let fileManager = FileManager.default
        LoggerService.shared.log(
            type: .info, module: "AIRACService", message: "fileManager: \(fileManager)")
        do {
            ...
            LoggerService.shared.log(
                type: .info, module: "AIRACService", message: "cachePath: \(cachePath)")
            if fileManager.fileExists(atPath: cachePath.path) {
                let cacheContents = try fileManager.contentsOfDirectory(
                    at: cachePath, includingPropertiesForKeys: [.creationDateKey])

                // 删除超过30天的缓存文件
                let thirtyDaysAgo = Date().addingTimeInterval(-30 * 24 * 60 * 60)

                for fileURL in cacheContents {
                    if let creationDate = try? fileURL.resourceValues(forKeys: [.creationDateKey])
                        .creationDate,
            ...
            LoggerService.shared.error(
                module: "AIRACService", message: "清理文件缓存失败: \(error.localizedDescription)")
        }
    }
    ...
    func getCacheSize() -> String {
        ...
        do {
            ...
            if fileManager.fileExists(atPath: cachePath.path) {
                let cacheContents = try fileManager.contentsOfDirectory(
                    at: cachePath, includingPropertiesForKeys: [.fileSizeKey])
            ...
            LoggerService.shared.error(
                module: "AIRACService", message: "获取缓存大小失败: \(error.localizedDescription)")
        }
                ...
                if !airacService.isUpdating {
                    VStack(spacing: 12) {
                        if airacService.errorMessage != nil {
                            Button("重试") {
                                Task {
                                    await airacService.checkAndUpdateAIRAC(
