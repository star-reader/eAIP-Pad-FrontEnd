...
struct PDFReaderView: View {
    ...
    var body: some View {
        ...
        .task(id: chartID) {
            LoggerService.shared.info(
                ...
                message: "task 触发 - chartID: \(chartID), documentType: \(documentType.rawValue)")
            await loadPDF()
        }
        .onChange(of: chartID) { oldValue, newValue in
            LoggerService.shared.info(
                module: "PDFReaderView", message: "chartID 变化: \(oldValue) -> \(newValue)")
            Task {
                await loadPDF()
            }
        }
        .onAppear {
            LoggerService.shared.info(
                module: "PDFReaderView", message: "onAppear - chartID: \(chartID)")
        }
        .onDisappear {
            LoggerService.shared.info(
                module: "PDFReaderView", message: "onDisappear - chartID: \(chartID)")
        }
    }
    ...
    private func togglePin() {
        if isPinned {
            ...
            // 获取当前AIRAC版本
            let currentAIRAC =
                PDFCacheService.shared.getCurrentAIRACVersion(modelContext: modelContext)
                ?? "unknown"
    ...
    private func sharePDF() {
        ...
        do {
            ...
            LoggerService.shared.error(
                module: "PDFReaderView", message: "创建临时文件失败: \(error.localizedDescription)")
        }
    }
    ...
    private func downloadPDF() {
        ...
        do {
            ...
            LoggerService.shared.error(
                module: "PDFReaderView", message: "保存PDF失败: \(error.localizedDescription)")
        }
    }
    ...
    private func printPDF() {
        ...
        DispatchQueue.main.async {
            printController.present(animated: true) { controller, completed, error in
                if let error = error {
                    LoggerService.shared.error(
                        module: "PDFReaderView", message: "打印失败: \(error.localizedDescription)")
                }
            }
        }
    }
                            ...
                            Button {
                                ...
                                VStack(spacing: 8) {
                                    ...
                                    PDFPageThumbnail(page: page)
                                        .aspectRatio(
                                            page.bounds(for: .mediaBox).width
                                        ...
                                        .overlay(
                                            RoundedRectangle(cornerRadius: 6)
                                                .stroke(
                                                    ...
                                                    lineWidth: currentPage == pageIndex ? 3 : 1)
                                        )
                                    ...
                                    Text("第 \(pageIndex + 1) 页")
                                        .font(.caption2)
                                        .foregroundColor(
                                            currentPage == pageIndex ? .blue : .secondary
                                        )
